...
class FlatTradeApiPy(NorenApi):
    def __init__(self, emulation=True):
        ...
        if not emulation:
            ...
            if token_is_invalid:
                ...
                # print(cred , res)
                # self.save_token(cred["userid"], cred["password"], res)
    ...
    def place_order(self, tradingsymbol, transaction_type, order_type, quantity, price=0.0, trigger_price=None, exchange="NFO"):
        if self.emulation:
            res = {
    ...
    def instruments(self, exchange="NFO"):
        ...
        df.dropna(axis=1, how="all", inplace=True)  # safer drop
    ...
    def format_candle_data_response(self, res, oi=False, return_df=False):
        ...
        col_to_be_drop = ["stat", "ssboe", "intoi", "v"]
        if not oi:
